...
class BaseProductAdmin(admin.ModelAdmin):
    list_display = ('display_name_jp', 'display_management_number_jp', 'display_image_jp', 'display_price_jp', 'display_timer_jp')
    ...
    def display_name_jp(self, obj):
        parts = re.split(r'[ _　]', obj.name)
        char_name = parts[0] if parts else obj.name
        return format_html('<div class="cell-center" style="font-weight: 800;">{}</div>', char_name)
    display_name_jp.short_description = '商品名'

    def display_management_number_jp(self, obj):
        parts = re.split(r'[ _　]', obj.name)
        mgmt = " ".join(parts[1:]) if len(parts) > 1 else "―"
        return format_html(
            '<div class="cell-center" style="font-size:11px; color:#aaa; font-weight:700;">{}</div>', mgmt
        )
    display_management_number_jp.short_description = '管理番号'
